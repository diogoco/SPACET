<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        //pesquisar na bd para verificar se existe conta de user com este email
        $dados = $gestor->EXE_QUERY('SELECT * FROM user WHERE email = :email',$parametros);

        //verificar se foi encontrado email
        if(count($dados) == 0){
            $erro = true;
            $mensagem = 'Não foi encontrada conta de user com esse email.';
        }
        
        //no caso de não haver erro (foi encontrada conta de user com o email indicado)
        else{
            ...
            if($mensagem_enviada){
                ...
                //LOG
                funcoes::CriarLOG('O user '.$dados[0]['nome'].' solicitou recuperação da password.', $dados[0]['nome']);
